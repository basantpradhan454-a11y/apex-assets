...
import { TrendingUp, TrendingDown, Wallet, Package, Star, Zap } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const totalChange = cards.reduce((sum, c) => {
    const change = ((c.market_price - c.previous_price) / c.previous_price) * 100
    return sum + change
  }, 0) / cards.length
  ...
  const stats = [
    {
      ...
      value: mode === 'demo' ? user?.virtual_balance?.toLocaleString() : user?.real_wallet_balance?.toLocaleString(),
      icon: Wallet,
      accent: 'gold',
    },
    {
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          {mode === 'demo' && (
            <span className="px-2 py-0.5 text-xs rounded-full bg-apex-gold/10 text-apex-gold gold-border">
              DEMO MODE
            </span>
          )}
        </div>
        <p className="text-apex-white-dim text-sm">
          {mode === 'demo'
            ? 'Trading with virtual credits. Complete KYC to unlock Live Trading.'
        ...
        {stats.map((stat, i) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className={`w-10 h-10 rounded-lg bg-apex-black-card flex items-center justify-center ${
                stat.accent === 'gold' ? 'gold-border' : ''
              }`}>
                <stat.icon
                  size={18}
                  className={
...
function PriceTicker({ cards }: { cards: typeof mockCards }) {
  return (
